...
from py.utils import read_image, cvt2Lab, upsample, cvt2rgb
...
def main():
    ...
    for gray_imagename in os.listdir(GRAY_IMAGE_PATH):
        gray_image, s = read_image(os.path.join(GRAY_IMAGE_PATH, gray_imagename))
    ...
    for epoch in range(EPOCH):
        ...
        for i in range(0, train_size, BATCH_SIZE):
            trainX, trainY = torch.autograd.Variable(torch.from_numpy(x_train[i:i + BATCH_SIZE]).float().cuda(),
                                                     requires_grad=False), \
            ...
            running_train_loss += train_loss.data

        for i in range(0, valid_size, BATCH_SIZE):
            validX, validY = torch.autograd.Variable(torch.from_numpy(x_valid[i:i + BATCH_SIZE]).float().cuda(),
                                                     requires_grad=False), \
            ...
            running_valid_loss += valid_loss.data

        print("%d\ttrain loss : %s\t%s" % (
        epoch + 1, str(format(running_train_loss / (x_train.shape[0] / BATCH_SIZE), '.8g')),
        str(format(running_valid_loss / (x_valid.shape[0] / BATCH_SIZE), '.8g'))))
    ...
    # Shape -> (100, 256, 256, 2)
    pred_valid = np.vstack([np.expand_dims(upsample(pred.astype(float)), axis=0) \
